 ...
 * @LastEditTime: 2022-10-11 10:14:03
...
void VelocityOptimizer::runSingleCubesPath(const std::vector<Cube2D<double>>& cube_path, const std::array<double, 3>& start_state, const double& end_s, const double& max_velocity, const double& min_velocity, const double& max_acceleration, const double& min_acceleration, int index) {
    ...
    // std::shared_ptr<OoqpOptimizationInterface> ooqp_itf = std::make_shared<OoqpOptimizationInterface>();
    // ooqp_itf->load(ref_stamps, start_state, end_s, unequal_constraints, equal_constraints, polynomial_unequal_constraints);
    // bool optimization_res = ooqp_itf->runOnce(&optimized_s, &objective_value);
